    ...
    a.Wexley.JustifiedGrid = a.Base.create("justified-grid", a.Plugin.Base, [], {
        ...
        _getGridData: function() {
            ...
            var a = this.get("gutter")
              ...
              , g = {};
            c.each(function(h, j) {
                var i = g, k, l = h.one("img");
                k = this.get("initialHeight");
                l = l.getAttribute("data-image-dimensions").split("x");
                k *= parseInt(l[0], 10) / parseInt(l[1], 10);
                i.width = k;
                g.el = h;
                f + g.width + a * (0 < d.items.length ? d.items.length - 1 : 1) <= this._containerWidth || 1 > d.items.length ? (f += g.width,
                d.items.push(g)) : (d.width = f,
                d.scale = this._calculatescale(d, f),
                e.push(d),
                d = {
                    items: []
                },
                d.items.push(g),
                f = g.width);
                j == c.size() - 1 && (d.width = f,
                d.scale = this._calculatescale(d, f),
                1.5 < d.scale && (i = e[e.length - 2],
                d.scale = i ? i.width * i.scale > this._containerWidth - i.items.length * a ? i.scale : 1 : 1),
                e.push(d));
                g = {}
            }, this);
            return e
        ...
        _calculatescale: function(a, c) {
            return (this._containerWidth - this.get("gutter") * (a.items.length - 1)) / c
        },
        _render: function(b) {
            var c = this.get("gutter"), e, d, f, g;
...
Y.use(["node", "squarespace-gallery-ng", "squarespace-image-loader", "event-key", "justified-grid"], function(a) {
    ...
    a.Wexley.Site = Singleton.create({
        ...
        initialize: function() {
            ...
            if (a.UA.mobile)
                a.one(a.config.win).on("orientationchange", function() {
                    this._initializeThumbs()
                }, this);
            if (a.one("body.collection-type-gallery"))
                this.setupGallery(),
        ...
        loadThumbs: function() {
            if (!a.one("body.full-view")) {
                ...
                }) : (b && b.justifiedgrid && b.justifiedgrid.destructor(),
                a.all("#thumbList img").each(function(a) {
                    a.setStyle("height", null).get("parentNode").setStyles({
                        marginRight: null,
                        height: null,
                        width: null
                    });
        ...
        setupGallery: function() {
            if (200 > a.one("body").get("winWidth"))
                a.all("#slideshow .slide").each(function(b) {
                ...
                var b = parseInt(a.Squarespace.Template.getTweakValue("outerPadding"), 10)
                  , c = parseInt(a.Squarespace.Template.getTweakValue("logoSize"), 10);
                ...
                var d = a.one("#simpleControls").get("offsetHeight") + a.one("#numberControls").get("offsetHeight") + a.one("#dotControls").get("offsetHeight") + a.one("#tinyThumbControls").get("offsetHeight") + 40
                  , f = function() {
                    ...
                    600 < c - b - 2 * e ? a.one("#slideshowWrapper").setStyle("height", c - 2 * b - e - d) : a.one("#slideshowWrapper").setStyle("height", "600px")
                };
                f();
                c = function() {
                ...
                a.one(window).on("key", function() {
                    a.one(".full-view") ? a.Squarespace.EscManager.disable() : a.Squarespace.EscManager.enable()
                }, "esc");
                e = a.bind(function() {
                ...
                c && (c = a.one('#thumbList .thumb[data-slide-id\x3d"' + c + '"]')) && c.simulate("click")
            }
        ...
        setupTweakHandlers: function() {
            a.Global.on("tweak:change", function(b) {
                ...
                "indexItemPadding" == c && (b = a.one("#thumbList"),
                b.justifiedgrid && (b.justifiedgrid.set("gutter", parseInt(b.one(".thumb").getComputedStyle("marginRight"))),
                b.justifiedgrid.refresh()));
